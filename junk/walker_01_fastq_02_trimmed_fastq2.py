...
# Rule order
# Quality control: run_fastqc > concatenate_fastq > cat_fastqc > cat_multiqc > trimmomatic > trimmed_fastqc > trimmed_multiqc
...
rule all:
	input:
		...
		# Does not include samples with only 1 run (see rule "cat_fastqc")
		# HTML files; Zip files with same naming structure are created but not used (no MultiQC by-run)
