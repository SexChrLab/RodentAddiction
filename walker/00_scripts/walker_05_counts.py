...
# 1) All tool paths
# 2) annotation_gtf
# 3) start_dir (your starting directory)
...
rule all:
	input:
		...
		expand(os.path.join(hisat2_stringtie_count_tran_dir, "{sample}/walker_{sample}_hisat2_stringtie_count_transcripts.gtf"), sample = config["30sal_ALL"]),
		expand(os.path.join(hisat2_stringtie_cov_tran_dir, "walker_{sample}_hisat2_stringtie_covered_transcripts.gtf"), sample = config["30sal_ALL"]),
		expand(os.path.join(hisat2_stringtie_gene_dir, "walker_{sample}_hisat2_stringtie_genes.txt"), sample = config["30sal_ALL"]),
		os.path.join(hisat2_stringtie_final_count_dir, "walker_gene_count_matrix.csv"),
		os.path.join(hisat2_stringtie_final_count_dir, "walker_transcript_count_matrix.csv"),
...
rule hisat2_stringtie_count:
	...
	output:
		TRANSCRIPT_GTF = os.path.join(hisat2_stringtie_count_tran_dir, "{sample}/walker_{sample}_hisat2_stringtie_count_transcripts.gtf"),
		COVERED_TRANSCRIPTS = os.path.join(hisat2_stringtie_cov_tran_dir, "walker_{sample}_hisat2_stringtie_covered_transcripts.gtf"),
		GENE_ABUNDANCES = os.path.join(hisat2_stringtie_gene_dir, "walker_{sample}_hisat2_stringtie_genes.txt")
	params:
...
rule run_prepDE:
	priority: 10
	input: 
		PREP_TXT = os.path.join(misc_dir, "walker_prepDE_30sal.txt") # Contains sample IDs and assembled transcript GTF locations
	output:
		GENE_MATRIX = os.path.join(hisat2_stringtie_final_count_dir, "walker_gene_count_matrix.csv"),
		TRANSCRIPT_MATRIX = os.path.join(hisat2_stringtie_final_count_dir, "walker_transcript_count_matrix.csv")
	params:
		gtf_dir = hisat2_stringtie_count_tran_dir,
		prepDE3 = prepDE_path,
		length = 100 # Average read length
	shell:
		"""
