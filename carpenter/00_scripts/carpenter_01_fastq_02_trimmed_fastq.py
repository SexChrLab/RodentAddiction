...
rule all:
	input:
		...
		os.path.join((misc_dir, "carpenter_1abs_multiqc_list.txt"),
		os.path.join((misc_dir, "carpenter_1abs_multiqc_list.txt"),
		# Initial quality control check on set of FASTQ files (MultiQC)
		os.path.join(fastqc_dir, "carpenter_1abs_multiqc.html"),
		...
		expand(trimmed_fastq_dir, "stats/carpenter_{sample}_bbduk_stats.txt"), sample = config["28abs_ALL"]),
		...
		# List of 28abs FastQC Zip files
		os.path.join(misc_dir, "carpenter_28abs_trim_multiqc_list.txt"),
...
rule initial_multiqc_setup:
	input:
		...
		expand(os.path.join(fastqc_dir, "carpenter_{sample}_fq2_fastqc.zip"), sample = config["28abs_ALL"])
	output:
		LIST_1ABS = os.path.join(misc_dir, "carpenter_1abs_multiqc_list.txt"),
		LIST_28ABS = os.path.join(misc_dir, "carpenter_28abs_multiqc_list.txt")
